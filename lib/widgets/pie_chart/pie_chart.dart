...
class PieChart extends StatefulWidget {
  const PieChart({Key? key, required this.data, required this.name})
...
class _PieChartState extends State<PieChart>
  ...
  Widget build(BuildContext context) {
    ...
    final Color shadowColor =
        context.isDark ? AppColors.dark_bg_gray : const Color(0x80C8DAFA);
    return Container(
            ...
            child: Center(
              child: ExcludeSemantics(
                child: Column(
                  ...
                  children: <Widget>[
                    Text(widget.name, style: AppTextStyles.textBold16),
                    Gaps.vGap4,
                    Text('$count件')
                  ],
                ),
...
class PieChartPainter extends CustomPainter {
  PieChartPainter(
    ...
    if (data.length == 11) {
      ...
      pieData.color = AppColors.text_gray_c;
      // 移除“其他”后，按数量排序
      data.removeAt(10);
    }
...
//      print(data[i].toString());
  ...
  void drawPercentage(
    ...
    final TextSpan span = TextSpan(
        style:
            const TextStyle(color: Colors.white, fontSize: AppDimens.font_sp12),
        text: percentage);

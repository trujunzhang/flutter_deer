...
import 'package:flutter_deer_djzhang/account/models/withdrawal_account_model.dart';
import 'package:flutter_deer_djzhang/account/widgets/withdrawal_account_item.dart';
...
class _WithdrawalAccountPageState extends State<WithdrawalAccountPage> {
      ...
      appBar: MyAppBar(
          ...
          onPressed: () {
            NavigatorUtils.pushResult(
                context, AccountRouter.addWithdrawalAccountPage, (result) {
              _insertItem(0);
            });
          }),
      body: _list.isEmpty
          ...
          : AnimatedList(
              ...
              itemBuilder: (_, index, animation) =>
                  sizeItem(_list[index], index, animation),
            ),
    );
  }

  Widget sizeItem(
      WithdrawalAccountModel data, int index, Animation<double> animation) {
    /// item插入、移除动画
  ...
  void _insertItem(int index) {
    final WithdrawalAccountModel item =
        WithdrawalAccountModel('weilu_deer', '微信', 1, '');
